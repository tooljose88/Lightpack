...
const char * ApiServer::CmdLoadPlugin = "load-plugin";
const char * ApiServer::CmdUnloadPlugin = "unload-plugin";
...
void ApiServer::clientProcessCommands()
{
    ...
    while (m_clients.contains(client) && client->canReadLine())
    {
        ...
        else if (cmdBuffer == CmdLoadPlugin){
            API_DEBUG_OUT << cmdBuffer; 	
            SetEnabledPlugins(true);
            result = CmdApiKeyResult_Ok;
        }
        else if (cmdBuffer == CmdUnloadPlugin){
            API_DEBUG_OUT << cmdBuffer; 	
